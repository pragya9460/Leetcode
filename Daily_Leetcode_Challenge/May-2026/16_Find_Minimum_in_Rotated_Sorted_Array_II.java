 ...
 * 
 * My youtube video of this question: https://youtu.be/LzFxh40PpVw
 * Leetcode Link: https://leetcode.com/problems/find-minimum-in-rotated-sorted-array-ii/
 */

// C++ code (Optimal -> Using Binary Search)
// Approach: Use binary search to locate the rotation point, which is the index of the minimum element. By comparing nums[mid] with nums[high], we determine whether mid lies in the left or right sorted portion; if both are equal (due to duplicates), we shrink the search space by doing high--
// TC: O(log(n))
// SC: O(1)
class Solution {
    public:
